...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("ui_accept") and held_ball:
		held_ball.freeze = false
		var hold_pos = global_transform.origin + $Pivot.transform.basis.z * -1.6 + Vector3(0, 0.4, 0)
	...
	if held_ball:
		var hold_pos = global_transform.origin + $Pivot.transform.basis.z * -1.2 + Vector3(0, 0.4, 0)
		held_ball.global_transform.origin = hold_pos
